...
import { UreelCardCore } from './UreelCardCore';
...
export const UreelStudioShell: React.FC<UreelStudioShellProps> = ({
  ...
  const setEndCardEnabled = async (enabled: boolean) => {
    await setEndCard({
      enabled,
      source: enabled ? ((((endCard as any).source) && (endCard as any).source !== 'none') ? (endCard as any).source : 'scene') as any : (endCard as any).source,
    });
  };
  ...
  const renderWerbeTextMonitor = (compact = false) => {
    ...
    // all go through UreelCardCore and therefore the same text renderer.
    return (
              ...
              {activeSubSection === 'buttons-preview' && (
                <div className="space-y-4">
                  <div className="bg-[#111111] p-4 rounded-2xl border border-[#3A3732] space-y-4">
                    ...
                    {buttonPreviewMode === 'card' && <div className="rounded-3xl border border-[#3A3732] bg-[#0B0B0B] p-3 max-w-[260px] mx-auto"><div className="h-[430px] rounded-[28px] overflow-hidden border-[8px] border-[#1C1C1C] bg-black"><UreelCardCore
                card={monitorCard}
                lang={lang}
