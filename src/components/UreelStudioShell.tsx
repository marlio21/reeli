...
export const UreelStudioShell: React.FC<UreelStudioShellProps> = ({
  ...
  const applyMobileButtonSizePreset = async (btnId: string, preset: 'compact' | 'standard' | 'large' | 'xlarge') => {
    ...
    const values = preset === 'compact'
      ? { px: 52, gap: 9, fontSize: 8.6, iconSize: 16, textPadding: 4, borderWidth: 'thin' as const, scale: 0.70, radius: 'rounded' as const }
      : preset === 'large'
        ? { px: 76, gap: 10, fontSize: 10.4, iconSize: 20, textPadding: 5, borderWidth: 'thin' as const, scale: 0.88, radius: 'rounded' as const }
        : preset === 'xlarge'
          ? { px: 88, gap: 10, fontSize: 11.4, iconSize: 22, textPadding: 5, borderWidth: 'thin' as const, scale: 0.96, radius: 'rounded' as const }
          : { px: 64, gap: 9, fontSize: 9.6, iconSize: 18, textPadding: 4, borderWidth: 'thin' as const, scale: 0.80, radius: 'rounded' as const };
                ...
                <div className="bg-[#111111] p-4 rounded-2xl border border-[#3A3732] space-y-4">
                  ...
                  {activeSubSection === 'buttons-size' && (
                    <div className="space-y-3">
                      ...
                      <input type="range" min={42} max={88} step={2} value={buttonSizePx} onChange={(e) => syncCardUpdate({ buttonSizePx: Number(e.target.value), buttonGridLayout: { ...(activeCard.buttonGridLayout || {}), buttonSizePx: Number(e.target.value) } as any })} className="w-full accent-[#E8DCC2]" />
                    </div>
                  )}
          ...
          return (
            <section className="ureel-tap-panel ureel-tap-panel--button">
              ...
              {tapButtonTool === 'look' && <div className="ureel-tap-config ureel-mobile-look-editor">
                ...
                <p className="ureel-tap-inline-hint">Klein 44px, Normal 52px, Groß 58px, Sehr groß 66px. Diese Werte werden direkt in alle Public-Layoutfelder geschrieben.</p>
