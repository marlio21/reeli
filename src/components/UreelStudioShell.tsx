...
export const UreelStudioShell: React.FC<UreelStudioShellProps> = ({
  ...
  const applyMobileButtonSizePreset = async (btnId: string, preset: 'compact' | 'standard' | 'large' | 'xlarge') => {
    // v52.5.40: safe mobile sizes with Sehr groß at 122px.
    ...
    const values = preset === 'compact'
      ? { px: 56, gap: 9, fontSize: 8.8, iconSize: 17, textPadding: 4, borderWidth: 'thin' as const, scale: 0.72, radius: 'rounded' as const }
      : preset === 'large'
        ? { px: 112, gap: 8, fontSize: 11.3, iconSize: 23, textPadding: 5, borderWidth: 'thin' as const, scale: 0.94, radius: 'rounded' as const }
        : preset === 'xlarge'
          ? { px: 122, gap: 8, fontSize: 11.9, iconSize: 24, textPadding: 5, borderWidth: 'thin' as const, scale: 0.97, radius: 'rounded' as const }
  ...
  const getMobileCardButtonTilePx = (value: any = buttonSizePx) => Math.max(38, Math.min(Number(value || 52), 122));
  ...
  const getCleanMonitorCard = (card: Card): Card => {
    ...
    return {
      ...
      buttonGridLayout: {
        ...
        cols: 3,
        square: true,
        buttonSizePx: Math.max(48, Math.min(122, Number((card.buttonGridLayout as any)?.buttonSizePx || (card as any).buttonSizePx || 72))),
        gapPx: Math.max(4, Math.min(18, Number((card.buttonGridLayout as any)?.gapPx || (card as any).buttonGapPx || 6))),
        align: 'center',
      } as any,
      buttonSizePx: Math.max(48, Math.min(122, Number((card as any).buttonSizePx || 72))) as any,
                ...
                <div className="bg-[#111111] p-4 rounded-2xl border border-[#3A3732] space-y-4">
                  ...
                  {activeSubSection === 'buttons-size' && (
                    <div className="space-y-3">
                      ...
                      <input type="range" min={42} max={122} step={1} value={buttonSizePx} onChange={(e) => syncCardUpdate({ buttonSizePx: Number(e.target.value), buttonGridLayout: { ...(activeCard.buttonGridLayout || {}), buttonSizePx: Number(e.target.value) } as any })} className="w-full accent-[#E8DCC2]" />
                    </div>
                  )}
          ...
          return (
            <section className="ureel-tap-panel ureel-tap-panel--button">
              ...
              {tapButtonTool === 'look' && <div className="ureel-tap-config ureel-mobile-look-editor">
                ...
                <p className="ureel-tap-inline-hint">Klein 56px, Normal 68px, Groß 112px, Sehr groß 122px. Diese Werte werden direkt in alle Public-Layoutfelder geschrieben.</p>
