  ...
  const applyMobileButtonSizePreset = async (btnId: string, preset: 'compact' | 'standard' | 'large' | 'xlarge') => {
    ...
    const values = preset === 'compact'
      ...
      : preset === 'large'
        ...
        : preset === 'xlarge'
          ? { px: 100, gap: 8, fontSize: 11.2, iconSize: 23, textPadding: 5, borderWidth: 'thin' as const, scale: 1.00, radius: 'rounded' as const }
                ...
                <div className="bg-[#111111] p-4 rounded-2xl border border-[#3A3732] space-y-4">
                  ...
                  {activeSubSection === 'buttons-size' && (
                    <div className="space-y-3">
                      ...
                      <input type="range" min={42} max={100} step={2} value={buttonSizePx} onChange={(e) => syncCardUpdate({ buttonSizePx: Number(e.target.value), buttonGridLayout: { ...(activeCard.buttonGridLayout || {}), buttonSizePx: Number(e.target.value) } as any })} className="w-full accent-[#E8DCC2]" />
                    </div>
                  )}
          ...
          return (
            <section className="ureel-tap-panel ureel-tap-panel--button">
              ...
              {tapButtonTool === 'look' && <div className="ureel-tap-config ureel-mobile-look-editor">
                ...
                <p className="ureel-tap-inline-hint">Klein 56px, Normal 68px, Groß 80px, Sehr groß 100px. Diese Werte werden direkt in alle Public-Layoutfelder geschrieben.</p>
