...
## v52.5.60 – Desktop Editor Precision Fix & Text Position Binding

- Added canonical hero text Y helpers (`getHeroTextY`, `buildHeroTextYPatch`) and wired editor, persistence and renderer through them.
- Reduced Desktop button navigation to Text / Aktion / Look / Verwalten.
- Limited Desktop button management and grid preview paths to the six real card buttons.
- Extended safe Look transfer whitelist with text color, icon styling and font look fields while preserving labels, actions, icons and IDs.
- Enlarged the active button inspection preview and renamed transfer actions to the requested Look/Design wording.
